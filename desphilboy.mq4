...
enum TradeActs {
    Initialize, Repair, Append, Terminate, NoAction
};
enum GroupIds {
...
// fibonacci
enum FiboRetrace {
    NoRetrace = 0, PaniclyRetrace = 1, MinRetrace = 2, LowRetrace = 3, HalfRetrace = 4, MaxRetrace = 5
};
double Fibo[] = {
    0.000, 0.05, 0.236, 0.382, 0.500, 0.618
};
...
int getPositionsInterval(string symbol, int operation, double rangeLow, double rangeHi, int & results[], bool spaceOpenPositions = false, int group = NoGroup) {
    ...
    for (int i = 0; i < OrdersTotal(); i++) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
            if (OrderSymbol() == symbol && (OrderType() == operation || (OrderType() == openPosType && spaceOpenPositions)) && OrderOpenPrice() > rangeLow && OrderOpenPrice() < rangeHi && (group == NoGroup || getGroup(OrderMagicNumber()) <= group || (OrderType() == openPosType && spaceOpenPositions))) {
                results[resultCounter] = OrderTicket();
                resultCounter++;
            }
        }
...
int getPositionsIntervalSameGroup(string symbol, int operation, double rangeLow, double rangeHi, int & results[], bool spaceOpenPositions = false, int group = NoGroup) {
    ...
    for (int i = 0; i < OrdersTotal(); i++) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
            if (OrderSymbol() == symbol && (OrderType() == operation || (OrderType() == openPosType && spaceOpenPositions)) && OrderOpenPrice() > rangeLow && OrderOpenPrice() < rangeHi && (group == NoGroup || getGroup(OrderMagicNumber()) == group)) {
                results[resultCounter] = OrderTicket();
                resultCounter++;
            }
        }
...
double getHighestLots(string symbol) {
    double highestLots = 0;

    for (int i = 0; i < OrdersTotal(); i++) {
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
            if (OrderSymbol() == symbol) {
                if (OrderType() == OP_BUY || OrderType() == OP_SELL) {
                    highestLots = MathMax(highestLots, OrderLots());
                }
            }
        }
    }
