...
int doPositions() {
    ...
    spacings[gid_VeryLongTerm] = VeryLongTermSpacing;
    spacings[gid_LongTerm] = LongTermSpacing;
    spacings[gid_MediumTerm] = MediumTermSpacing;
    spacings[gid_ShortTerm] = ShortTermSpacing;
    spacings[gid_VeryShortTerm] = VeryShortSpacing;
    ...
    for (int i = 0; i < OrdersTotal(); i++) {
        ...
        if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) {
            if (OrderSymbol() == symbol) {
                ...
                if (getGroup(OrderMagicNumber()) == VeryShortTerm) {
                    colour = VeryShortColour;
